...
def create_cost_functions(q_min, q_max, env, xd, tolerance=1e-3):
    """
    创建各种代价函数
...
def setup_optimization_parameters():
    ...
    # 优化算法参数
    opt.pop_size = 45    # CMA-ES种群大小：平衡探索能力和计算成本
    opt.max_iter = 200   # 最大迭代次数：充分优化
    opt.sigma_init = 1.5 # 初始标准差：较大的初始探索范围
    opt.log = True       # 启用日志记录以支持动画
    
    return opt


def run_trajectory_optimization(q0, q_min, q_max, env, xd, tolerance=1e-3):
    """
    运行轨迹优化
    ...
    # 创建代价函数
    cost_functions = create_cost_functions(q_min, q_max, env, xd, tolerance)
    
    print(f"Starting optimization from {q0} to x-target {xd}...")
    ...
    print(f"Target x-coordinate: {xd}")
    print(f"Target error: {abs(sol.w_best[-2] - xd):.6f}")
...
def plot_trajectory_results(q0, q_min, q_max, env, xd, pos, vel, acc, t_traj, save_figures=False):
    """
    绘制轨迹优化结果
    ...
    # 绘制目标线（垂直线表示目标x坐标）
    plt.plot([xd, xd], [q_min[1], q_max[1]], 'r--', linewidth=2, 
             label=f'Target x={xd:.3f}')
...
def animate_vpsto_evolution(traj_opt, sol, env, q0, dq0=None, qT=None, dqT=None,
    ...
    # 绘制目标线（红色竖线表示x约束）
    if xd is not None:
        ax.plot([xd, xd], [q_min[1] if q_min is not None else ax.get_ylim()[0], 
                          q_max[1] if q_max is not None else ax.get_ylim()[1]], 
                'r--', linewidth=2, alpha=0.8, label=f'Target x={xd:.3f}')
...
def main():
    ...
    figures = plot_trajectory_results(
        q0, q_min, q_max, env, xd, pos, vel, acc, t_traj, 
