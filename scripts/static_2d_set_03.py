...
def run_trajectory_optimization(q0, q_min, q_max, env, qd, tolerance=1e-3, prior=None):
    """
    运行轨迹优化
    ...
    # 创建VP-STO优化器
    traj_opt = VPSTO(opt)
    traj_opt.set_initial_guess(prior)
...
def main():    
    ...
    # 目标位置（二维坐标）
    qd = np.array([0.35, 0.25])
    tolerance = 1e-3  # 目标到达容忍度
    
    # 设置先验点，当任务太难不好收敛的时候可以用
    q_prior = None
    # q_prior = np.array([
    #     -0.05, 0.35,
    #     0.0, 0.42,
    ...
    traj_opt, sol, cost_functions = run_trajectory_optimization(
        q0, q_min, q_max, env, qd, tolerance, prior=q_prior
    )
