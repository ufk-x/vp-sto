"""
与 mpc_7d_panda_04.py 的区别是：
- 不使用 IK，完全靠采样自己探索 qT
- 任务空间目标可调（自己改）
- 新增：用一组“跟随 link 的小球”可视化 Panda 的几何近似（collision spheres）
...
from benchmarks.utils import (
    ...
    create_sphere, STATIC_MASS,                        # ⬅️ 新增：创建静态球
)
...
class CollisionSpheresVis:
    """
    把 {'link_name': [{'center':[x,y,z], 'radius':r}, ...], ...}
    渲染成随 link 运动的小球集合（静态刚体，用 resetBasePositionAndOrientation 跟随）。
    """
    ...
    def add_from_spec(self, spec: dict):
        for link_name, spheres in spec.items():
            link = link_from_name(self.robot, link_name)  # 名称->索引
            local_centers, radii, ids = [], [], []
            for s in spheres:
                c = np.asarray(s["center"], dtype=float)
                r = float(s["radius"])
                sid = create_sphere(r, mass=STATIC_MASS, color=self.color)  # 静态球
                p.setCollisionFilterGroupMask(sid, -1, collisionFilterGroup=0, collisionFilterMask=0)
                p.changeDynamics(sid, -1, lateralFriction=0, spinningFriction=0, rollingFriction=0,
...
class PredictiveSamplingController7D_TS:
    ...
    def predictive_sampling(self, q, dq):
        # 注意：qT=None, Q=None，dqT=0；qT 将“包含在参数 p 里”由采样自己探索。
...
if __name__ == "__main__":
    ...
    # --- 工具端 link 索引 ---
    tool_link = env.get_tool_link(env.robot)  # 'panda_grasptarget' 链接位姿作为 EEF
    ...
    # --- 任务空间目标（示例：直接指定）---
    # 也可以改成：x_start = np.array(env.get_eef_pose(env.robot)[:3]); xg = x_start + [0.15, 0.0, 0.30]
    xg = np.array([0.55, 0.00, 0.90], dtype=float)
    env.render_pose(xg.tolist())  # 可视化目标坐标系（仅位置用）

    # --- 创建“碰撞小球”可视化，并加载规格 ---
    # 选择从COLLISION_MODELS_PATH底下的franka.yml文件加载
    with open(f"{COLLISION_MODELS_PATH}/franka.yml", 'r') as f:
        collision_spheres = yaml.safe_load(f)
    spheres_vis = CollisionSpheresVis(env.robot, color=(0.2, 0.6, 1.0, 0.6))  # 天蓝半透明
    spheres_vis.add_from_spec(collision_spheres)
    ...
    while p.isConnected() and total_time < sim_duration:
        ...
        if acc + 1e-9 >= dt_control:
            ...
            p.addUserDebugLine(eef_prev, eef_now, lineWidth=2, lineColorRGB=GREEN[:3])
